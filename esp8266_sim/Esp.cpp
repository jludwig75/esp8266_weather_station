...
void EspClass::wdtEnable(uint32_t timeout_ms)
{
    ...
    assert(0);// This may need to be simulated or just removed: system_soft_wdt_restart();
}
...
void EspClass::wdtDisable(void)
{
    ...
    assert(0);// This may need to be simulated or just removed: system_soft_wdt_stop();
}

void EspClass::wdtFeed(void)
{
    assert(0);// This may need to be simulated or just removed: system_soft_wdt_feed();
}
...
{
    // This may need to be simulated or just removed: system_deep_sleep_set_option(static_cast<int>(mode));
    // This may need to be simulated or just removed: system_deep_sleep(time_us);
    // esp_yield();
    // TODO: Until we have something interesting to do to simulate deep sleep
    printf("EspClass::deepSleep called. Exiting\n");
    exit(0);
}
...
{
    assert(0);// Not yet simulated:  __real_system_restart_local();
}

void EspClass::restart(void)
{
    assert(0);// This may need to be simulated or just removed: system_restart();
    esp_yield();
}

uint16_t EspClass::getVcc(void)
{
    assert(0);// This may need to be simulated or just removed: InterruptLock lock;
    return 0;// system_get_vdd33();
}

uint32_t EspClass::getFreeHeap(void)
{
    assert(0);// This may need to be simulated or just removed: 
    return 1024 * 1024 * 1024;// system_get_free_heap_size();
}

uint32_t EspClass::getChipId(void)
{
    assert(0);// This may need to be simulated or just removed: 
    return 0;// system_get_chip_id();
}

const char * EspClass::getSdkVersion(void)
{
    return "1.0";// system_get_sdk_version();
}

uint8_t EspClass::getBootVersion(void)
{
    return 0;// system_get_boot_version();
}

uint8_t EspClass::getBootMode(void)
{
    return 0;// system_get_boot_mode();
}

uint8_t EspClass::getCpuFreqMHz(void)
{
    return 80;// system_get_cpu_freq();
}


uint32_t EspClass::getFlashChipId(void)
{
    return 0;// spi_flash_get_id();
}

uint32_t EspClass::getFlashChipRealSize(void)
{
    return 4 * 1000 * 1000; // (1 << ((spi_flash_get_id() >> 16) & 0xFF));
}
...
//#endif
    return 50000;// freeSpaceEnd - freeSpaceStart;
}
