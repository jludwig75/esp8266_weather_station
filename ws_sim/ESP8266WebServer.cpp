...
ESP8266WebServer *g_the_web_server; // hack because retarded programmers don't really understand C++ or OOP!
...
, _contentLength(0)
{
    assert(g_the_web_server == NULL);
    g_the_web_server = this;
}
...
ESP8266WebServer::~ESP8266WebServer() {
  ...
  close();
  delete _server;
  g_the_web_server = NULL;
}

void ESP8266WebServer::Get(webserver::http_request* r) {
    g_the_web_server->handle_get(r);
}


void ESP8266WebServer::begin() {

    _server = new webserver(_port, Get);
...
void ESP8266WebServer::handle_get(webserver::http_request* r)
{
    ...
    if (stricmp(r->method_.c_str(), "GET") == 0)
    {
        _currentMethod = HTTP_GET;
    }
    else if (stricmp(r->method_.c_str(), "POST") == 0)
    {
        _currentMethod = HTTP_POST;
    }
    else if (stricmp(r->method_.c_str(), "PUT") == 0)
    {
        _currentMethod = HTTP_PUT;
    }
    else if (stricmp(r->method_.c_str(), "OPTIONS") == 0)
    {
        _currentMethod = HTTP_OPTIONS;
    }
    else
    {
    ...
    _currentUri = r->path_.c_str();

    _currentArgCount = r->params_.size();
    _currentArgs = new RequestArgument[_currentArgCount];
    size_t i = 0;
    for (std::map<std::string, std::string>::const_iterator it = r->params_.begin(); it != r->params_.end(); ++it, ++i)
    {
        _currentArgs[i].key = it->first.c_str();
        _currentArgs[i].value = it->second.c_str();
    }
    ...
    for (_currentHandler = _firstHandler; _currentHandler != NULL; _currentHandler = _currentHandler->next())
    {
        if (_currentHandler->canHandle(_currentMethod, _currentUri))
        {
            handled = true;
            if (!_currentHandler->handle(*this, _currentMethod, _currentUri))
            {
                r->status_ = "500 Internal Server Error";
            ...
            // Don't allow more than one handler to handle it.
            break;
        }
    }
    ...
    if (!handled)
    {
        ...
        std::string title = "404 Not Found";
...
void ESP8266WebServer::sendContent_P(PGM_P content) {
    ...
    while (content != NULL) {
        ...
        assert(_currentRequest);
        _currentRequest->answer_ = std::string(contentUnit, contentUnit + contentUnitLen);
    }
}
